...
### 硬盘分块
使用硬盘和使用内存有一个很大的区别，内存可以支持到字节级别的随机存取，而这种情况在硬盘中通常是不支持的。一次读/写一个块（Block）才是可行的方案。

操作系统会将磁盘分成很多相等大小的块。这样做还有一个好处就是如果你知道块的序号，就可以准确地计算出块的物理位置。
...
FAT 的缺点就是非常占用内存，比如 1T 的硬盘，如果块的大小是 1K，那么就需要 1G 个 FAT 条目。

### 索引节点（Inode）
优势：
索引节点设计通过以下方式解决了FAT的问题：

1. 按需加载 ：只有当文件被访问时，其对应的inode才会被加载到内存中
2. 高效存储 ：每个文件有自己的inode，而不是维护整个文件系统的分配表
3. 支持大文件 ：通过多级索引结构支持超大文件
结构设计：
一个典型的inode包含：

- 文件属性信息 ：如创建时间、修改时间、访问权限、文件大小等
- 直接数据块指针 ：直接指向文件数据所在的物理块
- 间接指针 ：当文件较大时，指向包含更多块地址的指针块
...
### 解决性能和故障：日志文件系统 
日志文件系统通过记录变更而非直接修改数据来解决上述问题：
 
#### 性能优势：
- 虽然产生数据冗余，但读取速度不会显著下降（多从缓存读取）
- 写入速度大幅提升（多数时间写入内存，批量写入磁盘）
- 可以在内存缓冲区中积累一批日志后一次性写入磁盘
#### 容错优势：
- 丢失日志只会丢失近期变更，而非永久损坏数据
- 例如：丢失一天订单变更可通过第三方支付记录恢复，但随机删除订单则难以修复
#### 还原点（Checkpoint）机制：
为进一步提高可靠性，日志文件系统引入还原点机制：

1. 系统将固定时间段（如30秒）内的日志写入一个连续区域（还原点）
2. 写入开始时将还原点标记为"红色"，完成后标记为"绿色"
3. 读取文件时，系统依次读取还原点数据：
   - 绿色还原点：应用其中的日志
   - 红色还原点：丢弃（表示写入未完成）
...
行存储将表格看作一个个记录，每个记录是一行。  
行存储更新一行的操作，往往可以在一个块（Block）中进行。而查询数据，聚合数据，往往需要跨块。因此，行存储优点很明显，更新快、单条记录的数据集中，适合事务。但缺点也很明显，查询慢。

在列存储中同一列数据总是存放在一起，比如要查找某个时间段，很有可能在一个块中就可以找到，因为时间是集中存储的。

### 索引

### B 树和 B+ 树

## HDFS 介绍：分布式文件系统是怎么回事？

### 总结
Google 需要的是一个分布式数据库，存储的数据是包括内容、外链、Logo、标题等在内的网页的全部版本和描述信息。为了描述这些信息，一台机器磁盘不够大，吞吐量也不够大。因此 Google 需要将数据分布存储，将这个大表（BigTable）拆分成很多小片（Tablet）。当然，这并不是直接面向用户的架构。给几十亿用户提供高效查询，还需要分布式计算，计算出给用户使用的内容索引。
...
VMware 和 Docker 是目前虚拟化技术中最具代表性的两种设计。VMware 为应用提供虚拟的计算机（虚拟机）；Docker 为应用提供虚拟的空间，被称作容器（Container）

### 虚拟机的设计
* 隔离：一台实体机上的所有的虚拟机实例不能互相影响
* 仿真：指的是用起来像一台真的机器那样，包括开机、关机，以及各种各样的硬件设备
* 高效：的目标是减少虚拟机对 CPU、对硬件资源的占用。通常在虚拟机上执行指令需要额外负担10~15% 的执行成本，这个开销是相对较低的。因为应用通常很少将 CPU 真的用满，在容器中执行 CPU 指令开销会更低更接近在本地执行程序的速度。

为了实现上述的三种诉求，最直观的方案就是将虚拟机管理程序 Hypervisor 作为操作系统，在虚拟机管理程序（Hypervisor）之上再去构建更多的虚拟机。
...
为了实现世界切换，虚拟机上的操作系统需要使用硬件设备，比如内存管理单元（MMR）、TLB、DMA 等。这些设备都需要支持虚拟机上操作系统的使用，比如说 TLB 需要区分是虚拟机还是实体机程序。虽然可以用软件模拟出这些设备给虚拟机使用，但是如果能让虚拟机使用真实的设备，性能会更好。

### Type-2 虚拟机

Type-1 虚拟机本身是一个操作系统，所以需要用户预装。为了方便用户的使用，VMware 还推出了 Type-2 虚拟机
...
### 容器的设计
虚拟机虚拟的是计算机，容器虚拟的是执行环境。每个容器都是一套独立的执行环境，容器直接被管理在操作系统之内，并不需要一个虚拟机监控程序。
...
从逻辑上讲，系统的拆分，应该结合公司部门组织架构的调整，反映公司的战斗结构编排。但总的来说，互联网上的服务越来越复杂，几个简单的接口就可能形成一个服务，这些服务都要上线。如果用实体机来承载这些服务，开销太大。如果用虚拟机来承载这些服务倒是不错的选择，但是创建服务的速度太慢，不适合今天这个时代的研发者们。
 
为了保证微服务之间是隔离的，且可以快速上线。每个微服务我们都使用一个单独的容器，而一组容器，又包含在一个虚拟机当中
